"""
Test circuit transformation: PCB conversion and DAG analysis.
"""

import traceback
import numpy as np

from harvest.compilation.pauli_block_conversion import convert_to_PCB, create_dag, create_random_circuit, mqt_bench_pipeline
...
def test_convert_to_PCB(qc, visualize=False, analyze_layers=True):
    ...
    try:
        ...
        if analyze_layers:
            analyze_dag_layers(original_dag, "Original Circuit DAG Analysis")
        ...
        if analyze_layers:
            analyze_dag_layers(pcb_dag, "PBC Circuit DAG Analysis")
